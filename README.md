...
- **Learning from your fixes** ("Learn every word I correct", on by default): after a dictation lands, edit any inserted word normally. Voicey reviews each quiet edit burst immediately, stores replacements without the old spelling-similarity restriction, and shows a small `Learned “wrong” → “right”` card with **Undo**. Learning watches only the field Voicey just dictated into, for up to 60 seconds, never password fields. Pairs remain visible and removable in My Dictionary.
- **Never use the clipboard** (optional): on Android 13+, Voicey commits text through the accessibility service's active `InputConnection`, giving it keyboard-style insertion at the live cursor in rich editors such as Gmail without clipboard transit. Older devices fall back to direct accessibility writes; editors that reject those fail visibly instead of using a temporary paste.
- **Discard while recording:** while Voicey is listening, tap the small **×** inside the pill to stop and permanently discard that recording before it is transcribed or inserted. Tapping the waveform itself still stops and processes normally.
- **Smart app-aware formatting** (on by default): Voicey maps common email, messaging, notes, and document apps to conservative formatting profiles. It uses paragraphs for longer prose, bullets for genuine groups of points, and numbered lists for ordered steps—without forcing every dictation into a list. Email greetings and spoken closings receive deterministic paragraph separation, while focused-field hints keep subject, recipient, and search fields concise.
